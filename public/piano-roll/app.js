...
const state = { score:null, snapshot:null, draft:null, clipId:"", selected:-1, dirty:false, stale:false, saving:false, pendingSaves:new Set(), saveTimer:null, chasing:storedBoolean("shadowscore.pianoRoll.followSection",ui.chase.checked), folded:false, unfoldedScrollTop:null, drag:null, menuNote:null, orchestrationBusy:false, midiImport:null, playback:null, playbackGeneration:0, playbackRequest:null, wiperFrame:null, rnboTargets:[], timingContracts:[], dpr:Math.max(1,devicePixelRatio||1), left:58, top:22, lastBeatWidth:72 };
...
function storedBoolean(key,fallback){try{const value=localStorage.getItem(key);return value===null?Boolean(fallback):value==="true";}catch{return Boolean(fallback);}}
function storeBoolean(key,value){try{localStorage.setItem(key,String(Boolean(value)));}catch{}}
...
function activeBlockId(){const executionBlock=state.playback?.playing?executionBlockForVoice({voiceId:ui.player.value,assignment:state.score?.assignments?.[ui.player.value],targets:state.rnboTargets,contracts:state.timingContracts}):"";return executionBlock||state.playback?.activeBlockId||state.score?.structureState?.activeBlockId||"";}
function followChase(){const blockId=activeBlockId();if(!state.chasing||!blockId||blockId===ui.block.value||!blocks().includes(blockId))return false;if(state.drag||state.saving||activeDraftEntry()?.dirty)return false;populateSelectors(true);loadClip();resize();return true;}
...
async function saveNextClip(){if(state.saving)return;const clipId=[...state.pendingSaves][0];if(!clipId)return;state.pendingSaves.delete(clipId);const entry=draftStore.get(clipId);if(!entry?.dirty||entry.stale){if(state.pendingSaves.size)queueAutosave();return;}try{state.saving=true;markDirty();status(`Saving ${clipId}…`);const response=await fetch(`/clips/${encodeURIComponent(clipId)}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({clip:clone(entry.draft),expectedVersion:entry.baseVersion})});const payload=await response.json();if(!response.ok)throw new Error(payload.error||`HTTP ${response.status}`);state.score=payload;draftStore.saved(clipId,payload.clips[clipId],payload.version);draftStore.reconcile(payload);if(state.clipId===clipId)loadClip(true);status(`Saved ${clipId} at score revision ${payload.scoreRevision??payload.version}.`);render();}catch(error){const retrying=isScoreVersionConflict(error)&&await rebaseDraftAfterScoreChange(clipId,entry);if(retrying){state.pendingSaves.add(clipId);status(`Score position changed while saving ${clipId}; retrying automatically.`);}else{entry.stale=isScoreVersionConflict(error);status(`Autosave failed: ${error.message}`,true);}}finally{state.saving=false;markDirty();if(state.pendingSaves.size)queueAutosave();}}
function isScoreVersionConflict(error){return /stale score version/i.test(String(error?.message||error));}
async function rebaseDraftAfterScoreChange(clipId,entry){try{const response=await fetch("/score",{cache:"no-store"});if(!response.ok)return false;const score=await response.json();state.score=score;draftStore.reconcile(score);return draftStore.get(clipId)===entry&&entry.dirty&&!entry.stale;}catch{return false;}}
...
async function loadPlayback(){if(state.playbackRequest)return;const controller=new AbortController();state.playbackRequest=controller;try{const response=await fetch("/playback/snapshot",{cache:"no-store",signal:controller.signal});if(!response.ok)return;const snapshot=await response.json();const receivedAt=Date.now();if(!Number.isInteger(snapshot.generation)||snapshot.generation<=state.playbackGeneration)return;state.playbackGeneration=snapshot.generation;state.rnboTargets=Object.values(snapshot.targets||{});state.timingContracts=snapshot.timingContracts||[];state.playback={...(snapshot.playback||{}),playing:Boolean(snapshot.transport?.rolling??snapshot.transport?.running)};followChase();const displayBlockId=activeBlockId();const sampleBlockId=ui.block.value;const beat=playbackBeatForVoice({playback:state.playback,blockId:sampleBlockId,voiceId:ui.player.value,assignment:state.score?.assignments?.[ui.player.value],targets:state.rnboTargets,contracts:state.timingContracts});if(Number.isFinite(beat)){wiperEstimator.update({beat,tempo:snapshot.transport?.tempo,running:state.playback.playing,blockId:sampleBlockId,observedAt:snapshot.observedAt},receivedAt);}else wiperEstimator.clear();scheduleWiperFrame();ui.playing.textContent=Number.isFinite(beat)?`${sampleBlockId} · ${state.playback.playing?"":"client "}beat ${fmt(beat)}`:state.playback.playing?displayBlockId:"Stopped";}catch(error){if(error?.name!=="AbortError")console.warn("Playback snapshot refresh failed",error);}finally{if(state.playbackRequest===controller)state.playbackRequest=null;}}
...
ui.chase.checked=state.chasing;ui.block.disabled=state.chasing;
ui.chase.addEventListener("change",()=>{state.chasing=ui.chase.checked;storeBoolean("shadowscore.pianoRoll.followSection",state.chasing);ui.block.disabled=state.chasing;if(state.chasing)followChase();});
