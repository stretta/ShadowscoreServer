...
- `POST /transport/players/play`: start assigned players while preserving the requested Arrangement Run/Hold mode. If every assigned client is already advancing, Play adopts that external RNBO position, starts arrangement tracking at the observed block offset, and does not prepare, activate, reset, or rewrite client clocks. Matching active payload hashes are reported as verified; after a server restart, unknown payload provenance is preserved and surfaced as a warning rather than interrupting the running clients. Otherwise Play waits for queued RNBO score preparation and rejects current readiness failures; a failed redundant preparation does not override a matching healthy active payload. The conventional start path distributes the active block's TTID and Swing, reasserts runtime live tempo to the configured authority, starts JACK, writes `SetStage 0` to assigned clients by default, then sends `Clock 1` to assigned clients and `Clock: On` to resolved OSC sequencers. Playhead-only updates do not retransmit an already committed active block.
