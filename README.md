...
- `POST /transport/external`: accept an explicit `rolling` Play/Stop intent from a hardware controller such as Shadowbox. This adopts or releases arrangement ownership without preparing or replacing player payloads. On Play, fresh stage readbacks from every assigned client are required before phase writes; the server sends fleet-wide `Clock 0`, `SetStage <initiating address>`, and `Clock 1` phases so JACK's next received beat starts every armed client together. Direct timestamp-normalized readback must verify exact projected address alignment before arrangement tracking is marked synchronized. Missing or stale readback leaves synchronization explicitly degraded, and a missing initiating-client witness holds arrangement tracking instead of inventing a stage-zero anchor. RNBO stage readback remains execution verification and a fallback when an intent message is lost; it is not the normal handoff trigger.
