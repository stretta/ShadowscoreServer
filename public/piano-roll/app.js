...
const ui = { block:$("block"), player:$("player"), clip:$("clip"), grid:$("grid"), zoomX:$("zoom-x"), zoomY:$("zoom-y"), chase:$("chase"), fold:$("fold"), save:$("save"), revert:$("revert"), dirty:$("dirty"), editing:$("editing"), playing:$("playing"), selection:$("selection"), status:$("status"), roll:$("roll"), velocity:$("velocity"), rollScroll:$("roll-scroll"), velocityScroll:$("velocity-scroll"), velocityValue:$("velocity-value") };
const ctx = ui.roll.getContext("2d"); const vctx = ui.velocity.getContext("2d");
const draftStore=createClipDraftStore();
const state = { score:null, snapshot:null, draft:null, clipId:"", selected:-1, dirty:false, stale:false, chasing:false, folded:false, drag:null, playback:null, rnboTargets:[], timingContracts:[], dpr:Math.max(1,devicePixelRatio||1), left:58, top:22, minPitch:36, maxPitch:84 };
