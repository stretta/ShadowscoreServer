...
const ui = { block:$("block"), player:$("player"), clip:$("clip"), grid:$("grid"), zoomX:$("zoom-x"), zoomY:$("zoom-y"), fold:$("fold"), save:$("save"), revert:$("revert"), dirty:$("dirty"), editing:$("editing"), playing:$("playing"), selection:$("selection"), status:$("status"), roll:$("roll"), velocity:$("velocity"), rollScroll:$("roll-scroll"), velocityScroll:$("velocity-scroll"), velocityValue:$("velocity-value") };
const ctx = ui.roll.getContext("2d"); const vctx = ui.velocity.getContext("2d");
const draftStore=createClipDraftStore();
const state = { score:null, snapshot:null, draft:null, clipId:"", selected:-1, dirty:false, stale:false, folded:false, drag:null, playback:null, rnboTargets:[], timingContracts:[], dpr:Math.max(1,devicePixelRatio||1), left:58, top:22, minPitch:36, maxPitch:84 };
...
function markDirty(){const entry=activeDraftEntry();if(entry){state.dirty=entry.dirty;state.stale=entry.stale;}ui.save.disabled=!state.dirty||!state.clipId||state.stale; ui.revert.disabled=!state.dirty&&!state.stale;const otherDirty=draftStore.dirtyClipIds().filter(clipId=>clipId!==state.clipId);ui.dirty.textContent=state.stale?"Stale draft":state.dirty?"Unsaved clip":otherDirty.length?`${otherDirty.length} unsaved elsewhere`:"Saved";ui.dirty.title=otherDirty.length?`Unsaved: ${otherDirty.join(", ")}`:"";ui.dirty.className=`badge${state.stale?" stale":state.dirty||otherDirty.length?" dirty":""}`; }
