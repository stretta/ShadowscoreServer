...
- `GET /playback/snapshot`: monotonically versioned, server-owned playback observation combining the authoritative JACK playhead, macro/block state, cached local and peer RNBO execution witnesses, per-target phase error, readback freshness and observed stage movement, timing contracts, score-send preparation metrics, ACK state, and recent RNBO playback lifecycle events. The host polls advertised peer `currentStagePath` OSCQuery endpoints; browsers do not contact peers directly. Consecutive fresh stage changes distinguish externally running players from a merely available static readback.
...
- `POST /transport/players/play`: start assigned players while preserving the requested Arrangement Run/Hold mode. Known-offline assignments are retained but omitted from the participating cohort, so the arrangement can run in a clearly reported degraded state without that voice. Every available participant must still reach READY/ACTIVE and pass the existing clock acknowledgement and direct phase verification barriers; a participant that fails after cohort selection still rolls the coordinated start back. If every participating client is already advancing, Play adopts that external RNBO position, starts arrangement tracking at the observed block offset, and does not prepare, activate, reset, or rewrite client clocks. Matching active payload hashes are reported as verified; after a server restart, unknown payload provenance is preserved and surfaced as a warning rather than interrupting the running clients. Otherwise Play waits for queued RNBO score preparation and rejects participating-client readiness failures; a failed redundant preparation does not override a matching healthy active payload. The conventional start path distributes the active block's TTID and Swing, reasserts runtime live tempo to the configured authority, starts JACK, writes `SetStage 0` to the participating clients by default, then sends `Clock 1` to those clients and `Clock: On` to resolved OSC sequencers. Playhead-only updates do not retransmit an already committed active block.
...
- `POST /transport/external`: record an explicit `rolling` Play/Stop intent from an external hardware controller without changing Players or Arrangement ownership by default. A caller must send `adoptArrangement: true` to explicitly adopt or release arrangement ownership; adoption does not prepare or replace player payloads and explicitly selects Arrangement Run instead of consulting sticky Run/Hold state. On adopted Play, fresh stage readbacks from every assigned client are required before phase writes; the server sends fleet-wide `Clock 0`, `SetStage <initiating address>`, and `Clock 1` phases so JACK's next received beat starts every armed client together. Direct timestamp-normalized readback must verify exact projected address alignment before arrangement tracking is marked synchronized. Missing or stale readback leaves synchronization explicitly degraded, and a missing initiating-client witness holds arrangement tracking instead of inventing a stage-zero anchor. RNBO stage readback remains execution verification and a fallback when an adopted intent message is lost; it is not the normal handoff trigger.
