...
} from "../playback/playback-cohort-policy.mjs";
...
import { createSerializedOperationQueue } from "../playback/serialized-operation-queue.mjs";
...
export function createRnboOscAdapter(config, runtime = {}) {
  ...
  const activationOperations = createSerializedOperationQueue();
  ...
  const adapter = {
    ...
    async playbackUpdates(blockId = "", options = {}) {
      ...
      for (const update of updates) playbackUpdateState.observeAvailability(update);
      return {
    ...
    applyBlockUpdate(blockId = "", options = {}) {
      return activationOperations.run(() => runBlockUpdate(blockId, options));
    },
    activatePreparedBlock(blockId = "", options = {}) {
      return activationOperations.run(() => runPreparedBlockActivation(blockId, options));
    },
    sendStatus() {
  ...
  async function runPreparedBlockActivation(blockId = "", options = {}) {
    ...
    const blockStates = playbackUpdateState.values()
      ...
      .filter((state) => playbackUpdateState.availability(state.targetId)?.available !== false)
      .map((state) => {
    ...
    const cohort = evaluatePreparedPlaybackCohort({
      assignedVoiceIds: Object.keys(canonical.mesostructure?.[selectedBlockId]?.players ?? {}),
      states: blockStates,
      availability: playbackUpdateState.availabilityValues()
    });
    if (cohort.decision === "no-targets") {
      return {
        blockId: selectedBlockId,
        scoreRevision,
        state: "no-targets",
        ...
        unavailableTargetCount: cohort.unavailableTargetCount
      };
    }
    if (cohort.decision === "not-ready") {
      const error = new Error(`block '${selectedBlockId}' has no cached READY target cohort`);
      error.code = "PLAYBACK_UPDATE_NOT_READY";
      throw error;
    }
    if (cohort.decision === "already-active") {
      return {
        blockId: selectedBlockId,
        scoreRevision,
        state: "active",
    ...
    const preparedStates = cohort.preparedStates;
    ...
    const state = aggregatePlaybackUpdateState(Object.values(targets));
    return {
      blockId: selectedBlockId,
      scoreRevision,
      state,
      ...
      activationMode: "continue",
      boundary,
      action: activationActionForState(state),
      activations,
      fastPath: true
    };
  }
    ...
    try {
      ...
      if (!pending.length) {
        ...
        result = {
          ...finalUpdates,
          activationMode,
          action: activationActionForState(finalUpdates.state),
          activations
        };
      }
    } catch (error) {
  ...
  function recordPlaybackUpdate(target, compiled, status) {
    ...
    playbackUpdateState.recordDelivery({
      blockId,
      targetId,
      ...
      adapterTransactionId: prepared
        ? status?.preparedTransaction ?? compiled?.transactionId ?? null
        : active ? status?.activeTransaction ?? compiled?.transactionId ?? null : null,
      prepared,
      active,
      failed: compiled?.ack?.ok === false,
      error: compiled?.ack
    });
  }

  function promotePlaybackUpdate(targetId, transactionId, acknowledgement) {
    playbackUpdateState.promote(targetId, transactionId, acknowledgement);
  }

  function cachedPlaybackUpdates(blockId) {
    return playbackUpdateState.cached(blockId);
  }
