...
- `POST /hardware/register`: register a peer hardware unit and its RNBO targets. When a score assignment has a matching stable `deviceId`, registration safely refreshes stale `rnboTargetId`, host, port, and address fields if the peer advertises exactly one ShadowScore RNBO target; locked assignments, assignments without `deviceId`, and peers with multiple ShadowScore targets are left for manual selection. Targets may include `capabilities` such as `maxStages`, `maxNoteRows`, `noteDataFloatCount`, `noteRowWidth`, `contextDataFloatCount`, `supportedClockIntervals`, `supportsBeginReplaceClear`, `activeRowCountCommit`, and `compactScoreReplace`; registered peer targets that omit `capabilities` are treated as legacy clients unless the host config sets `rnbo.capabilities` after a verified fleet-wide client upgrade.
