...
- `POST /transport/external`: accept an explicit `rolling` Play/Stop intent from a hardware controller such as Shadowbox. This adopts or releases arrangement ownership immediately without preparing, replacing, resetting, or clock-writing player payloads. RNBO stage readback remains execution verification and a fallback when an intent message is lost; it is not the normal handoff trigger.
